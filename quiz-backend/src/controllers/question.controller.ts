...
export const uploadQuestionCSV = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.file) {
...
export const deleteBatchQuestions = async (req: AuthRequest, res: Response) => {
  try {
    ...
    res.status(200).json({ message: `${deletedCount} questions deleted` });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
};

export const deleteAllQuestions = async (req: AuthRequest, res: Response) => {
  try {
    const deletedCount = await QuestionService.deleteAllQuestionsBySchool(
      req.user!.id,
    );
    if (deletedCount === 0) {
      return res.status(400).json({ message: "No questions to delete" });
    }
    res.status(200).json({
      message: `${deletedCount} questions deleted`,
    });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
